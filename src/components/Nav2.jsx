...
const Navbar = () => {
    ...
    <div className="navbar-container">
        <div onClick={toggleMenu} className='fixed top-4 right-4 sm:top-8 sm:right-14 z-[71] flex items-center justify-between gap-2 sm:gap-3 p-3 sm:p-4 pl-4 sm:pl-6 pr-3 sm:pr-5 bg-[var(--color-primary)] border-b-[1px] border-[var(--color-contrast)] rounded-full transition-all duration-300 hover:scale-105 active:scale-95'>
            <div className="menu-text text-[var(--color-contrast)] font-montblanc text-lg sm:text-2xl mt-1">MENU</div>
            <div><svg className='arrow transition-transform duration-300 sm:w-5 sm:h-5' width="16" height="16"  viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {menuItems.map((item, index) => (
                    <div 
                        ...
                        style={{
                            ...
                            transition: isMenuOpen 
                                ? `transform 0.5s cubic-bezier(0.4, 0, 0.2, 1) ${0.3 + index * 0.1}s, opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1) ${0.3 + index * 0.1}s`
                    ...
                    >
                        ...
                        <span className="hover:opacity-60 transition-opacity duration-300">{item}</span>
                    </div>
                ))}
                <div 
                    ...
                    style={{
                        opacity: isMenuOpen ? 1 : 0,
                        transition: isMenuOpen 
                            ? `opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1) ${0.3 + menuItems.length * 0.1}s`
                            : 'opacity 0.2s ease-out'
                    }}
                >
                    <div>Dark / Light</div>
